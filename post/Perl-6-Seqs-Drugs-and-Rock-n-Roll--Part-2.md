...
those methods yourself is often when making an ``Iterator`` that's fed by
another ``Iterator``, as we'll see.
...
There are two more optimization methods possible: ``.bool-only``
and ``.count-only``. The first one returns `True` or `False`, depending on
whether there are still items that can be generated by the ``Iterator``. The
...
once per iteration of our ``Iterator`` until its ``tail`` is its ``head``.
...
We take an ``Iterable`` (a ``Range`` object out of which we can milk 5000
