...
In PART I of this article we'll talk about what `Seq`s are and how to make
them without the sequence operator. In PART II, we'll look at the thing-behind-
the-curtain of `Seq`'s: the `Iterator` type and how to make `Seq`s from our
own `Iterator`s. Lastly, in PART III, we'll examine the sequence operator in
all of its glory.
...
The `gather` statement and `take` routine are similar to "generators" and "yield" statement in some other languages:
    ...
    my $seq-full-of-sunshine := gather {
        ...
        take 'Taaake meee awaaay';
        say  'To better days'    ;

        take 'TAAAKEE MEEE AWAAAAYYYY';
        say  'A hiding place';
    }

Above, we have a code block with some [song lyrics](https://www.youtube.com/watch?v=0btXhLdAuAc), some of which we
[`say`](https://docs.perl6.org/routine/say) and others we
[`take`](https://docs.perl6.org/routine/take). Just like, `.say` can be used
as a method, you can use `.take` as a method, there's no real difference.
...
Now, let's iterate over `$seq-full-of-sunshine` and watch the output:
    ...
    # To better days
    # ▬▬▶ Entering
    # ❚❚ TAAAKEE MEEE AWAAAAYYYY
    # ◀▬▬ Leaving
    # A hiding place

Notice how the `say` statements we had inside the `gather` statement didn't
actualy get executed until we iterated over the value that `take` routines
took. The block got stopped and then continued only when more values from
the `Seq` were requested.

## That's exceptional!

The `take` routine works by throwing a `CX::Take`
[`CONTROL`](https://docs.perl6.org/syntax/CONTROL) exception that will
percolate up the call stack until something takes care of it. This means you
an feed a `gather` not just from an immediate block, but from a bunch of different sources, such as routine calls:
