...
*you* should do design. I'll tell you how *I* do it and if at the end you
...
appearance of new technologies and deprecation of old ones.
...
of it would you leave non-secure?
...
The cause for that is your design is too detailed on the code and not enough
...
instructions that even an idiot could follow and end up with a program.
...
900 pixel widths... or even 1000px, when I was feeling especially rebellious.
...
there are another two that are more obvious: tests and user documentation.
...
don't falsely blow up every time you make a change.
...
call `my $t = $weather-object.temp` and
...
use for our function could look like this:
...
Great! The interface is done. And the best thing is we can add extra methods
to the object we return, to add useful functionality, which brings me to the
next part:

### Overengineering

It's easy for programmers to overengineer their software. Unlike building
...
adding any measurable amount of usefulness.

Some examples are:

* Accepting multiple types of input (Array, Hash, scalars),
...
Note that none of the above features are inherently bad. It's the reasons for why they are added that suck. All of those items make your program more complex, which translates to: more bugs, more code to maintain, more code
...
cost of having it included.

My favourite example of overengineering
is [WeChall](https://www.wechall.net/) wargaming website. I'm pretty sure there's a button that will make that site mow my lawn... I just have to find
it first:

![](/assets/stock/20160525-Workshop-Part-2.png)

If I have some "cool" ideas for what my module XYZ can do, I usually simply
make sure they're possible to add with my current design, and then... I leave
them alone until someone asks me for them.

An astute reader will notice our `weather-for` can only do metric units
...
entire documentation fits onto half a screen.

If in the future `weather-for` needs to return Imperial units, we'll simply
make it accept `:imperial` positional argument that will switch it into
Imerial units mode. If we ever need wind direction as well, no problem,
just add it as an extra method in `Weather::Result` object.

Do less. Be lazy. In programming, that's a virtue.

## Our Repo

Our repository now contains completed `DESIGN.md` file with our design.
Commit what we wrote today:

    git add DESIGN.md
    git commit -m 'Write detailed design'
    git push

I created [a GitHub repo for this
project](https://github.com/zoffixznet/perl6-Workshop-1-Weatherapp), so you
can follow along and ensure you have all the files.

## Homework

Amend the design to include either of these features (or both):
(1) make it possible for `weather-for` to use both metric or Imperial units, depending on what the user wants; (2) Make it possible to give `weather-for`
actual names for countries rather than [ISO country
codes](http://www.nationsonline.org/oneworld/country_code_list.htm).

If you're feeling particularly adventurous, design a Web application
that will use our module.

## Conclusion

Today, we've learned how to think about the design of software before we
