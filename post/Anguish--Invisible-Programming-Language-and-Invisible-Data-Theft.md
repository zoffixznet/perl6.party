...
*Anguish* is based on [Brainf#%k](https://en.wikipedia.org/wiki/Brainfuck) (BF) except instead of using visible characters, it uses invisible ones. This
also means we can easily convert any BF program into an *Anguish* one using
...
For the interpreter, I chose the awesome Perl 6 programming language
...
You can [get the full distro in my repo](https://github.com/zoffixznet/perl6-Acme-Anguish/). Here, I'm replicating the main code:
...
On line 5 (and 55-64), we simply check our loops are matching. Lines 7-14
...
to the operator being processed.

One thing to note is lines 16-18, as well as line 57. You'll notice the
curious use of [`.NFC` method](http://docs.perl6.org/routine/NFC#class_Uni).
It converts our input code into Normal Form Composed.

Perl 6 has advanced Unicode support and, under normal use, the characters we're
...
`.grep` on line 16, had I used a regex, [as in my BF interpreter](https://github.com/zoffixznet/perl6-Inline-Brainfuck/blob/4e7a6739a544df079baa8a42d149b1ff1f0e7078/lib/Inline/Brainfuck.pm6#L16). To avoid the creation of graphemes, I used `eq` against a
[Junction](http://docs.perl6.org/type/Junction) instead.

This wraps it up for the *Anguish* language and those with intent can go and
try to write a full-featured browser in it now. As for the rest of us, let's
abuse our invisible Unicode chars some more and steal some data!

## PART II: Invisible Data Theft

The beauty of the invisible *Anguish* characters we used is they aren't
...
Here is where I placed the INVISIBLE SEPARATOR character that produced
the effect:
...
It's a debug helper and it just prints into a DEBUG.txt file. Feels like
something that could easily slip in. Once again, I'm using
`U+2063` character for the name of the operator.
Alright, now we're set to steal some data from an important piece of code:
...
As with the earlier example, I've inserted `U+2063` character right before
...
It's true, the change we made in `ReallyImportantAndSecretCode.p6` *will* show
...
of invisible characters to text looks like on GitHub:

![](/assets/stock/20160516-Anguish-language.jpg)

## Conclusion

*Anguish* is a language for true computer masochists who would love to question whether their program actually exists. However, the language does point out to us the reality of existence of Unicode characters that make sense
...
in other domains, such as programming languages.