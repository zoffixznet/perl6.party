...
still reference the proto, not the last `meow` method. Is there anything that
we can do?
...
[`Capture`](https://docs.perl6.org/type/Capture), it returns a list of
candidates that can handle it, with the narrowest candidate first in the list,
and since the returned object is a `Code`, we can query
...
The `$wanted` contains the setting line number given to us by `.line` call
and the `$cur-line-num` contains
the number of the current line we're examining. We loop until the
`$cur-line-num` reaches
...
`$file` and store the `$offset` of the first line of the code in that file.
Once done, we simply subtract the `$offset` from the setting
...
split on `g`. But, take a look at `$*VM.version`, which is the version of the
virtual machine we're running the code on. There aren't any `g`s and
...
That gives us the correct location of the `proto`. We can either pop
...
However, if you do actually run that code, after some output you'll be greeted
with this error:
...
The class it mentions is not a pure `Method` object, but has a mixin in it.
While `CoreHackers::Sourcery` recomposes all core subclasses of `Code` class
after augmenting it,
...
`use` a module and call subs and then copy/paste URLs or filenames.
You'll notice `sourcery` returns a list of two items: the filename and the URL.
This means we can make some nice and short
[aliases](http://askubuntu.com/questions/17536/how-do-i-create-a-permanent-bash-alias)
...
We `EVAL` the argument we give to these aliases, so be careful with them.
For `sourcery` alias, we [`run`](https://docs.perl6.org/routine/run) the Atom
...
`file:line-number` format to open files at a particular spot; if yours doesn't,
modify the command.

For `sourcery-web` we use the URL returned by `sourcery` and open Firefox
...
We've learned where Rakudo's source lives, how to find the commit
...
the setting generation script to *not* skip any lines, so making adjustments
